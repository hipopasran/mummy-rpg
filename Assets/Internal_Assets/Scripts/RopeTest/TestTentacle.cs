...
namespace Secret
{
    public class TestTentacle : MonoBehaviour
    {
        ...
        [SerializeField]private Transform ObjectToHock;
        ...
        {
            Debug.LogError(solver.positions[0]);
            ...
            private IEnumerator AttachHook()
    {
        ...
        Vector3 localHit = rope.transform.InverseTransformPoint(ObjectToHock.position);
        // Vector3 localHit = ObjectToHock.position;
        ...
        while (true)
        {
            ...
            // cube2.transform.position = ObjectToHock.position;

            // update direction and distance to hook point:
            direction = ObjectToHock.position - origin;
            float distance = direction.magnitude;
            direction.Normalize();
            
            Debug.LogError(distance);

            // LayParticlesInStraightLine(origin, direction.normalized);
        ...
        // batch.AddConstraint(rope.elements[0].particle1, character, transform.position, Quaternion.identity, 0, 0, float.PositiveInfinity);
